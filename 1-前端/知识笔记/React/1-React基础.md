...
**Vite 官方模板特点：** 仅提供 React + TypeScript + Vite，不含 ESLint/Prettier、Husky、测试等。需要完整工具链（ESLint、Prettier、Husky、lint-staged 等）时，可在 GitHub 搜索社区模板（关键词如 `vite react typescript eslint prettier`）直接使用，或在 Vite 项目基础上按各工具官方文档自行安装配置。
...
将 React 内容渲染到页面需三步：导包（`react-dom/client`）、创建根对象（`createRoot`）、调用根的 `render` 渲染内容。
...
> 💡 要渲染什么内容，直接写类似 HTML 的结构即可，这种写法叫做 **JSX**（写在 JS 中的 HTML）。现代构建工具（Vite、Next.js 等）默认启用新 JSX 转换，**无需 `import React`** 即可使用 JSX。

**StrictMode（严格模式）：**

脚手架生成的入口文件通常用 `<StrictMode>` 包裹根组件。它**仅在开发环境**生效，不影响生产构建：
...
| 行为 | 说明 |
| ---- | ---- |
| **组件函数执行两次** | 每次渲染时组件函数会被调用两次（第二次的结果被丢弃），用于暴露不纯的渲染逻辑（如意外的副作用） |
| **Effect 执行两次** | `useEffect` 的 setup → cleanup → setup，用于检测是否正确清理副作用 |
| **检测废弃 API** | 使用已弃用的生命周期方法或 API 时会在控制台警告 |

> **注意**：开发时看到 `console.log` 打印两次、请求发两次等，通常就是 StrictMode 导致的，**不是 Bug**。生产环境不会出现双重执行。
...
JSX 是 Meta 维护的**独立语法规范**，仅定义语法，不定义语义，也不是 ECMAScript 标准的一部分。React 只是 JSX 最广泛的使用者——React 生态里 JSX 被转成 `React.createElement(...)`，而 Vue、Solid 等也可用同一套 JSX 语法，由各自编译器转成各自的运行时调用。

### 4.3 JSX 原理（了解即可）

JSX 经构建工具转换后，会变为对 `react/jsx-runtime` 中 **`jsx()`** 函数的调用，得到描述 UI 的 JS 对象（形如 `{ type, props }`），用于后续虚拟 DOM 与渲染。这套转换称为 **Automatic Runtime（新 JSX 转换）**，React 17 起引入，Vite、Next.js 等现代工具默认启用。

```
JSX 源码 → 构建工具转换 → jsx(type, props) → 返回 { type, props } 对象树
```

`jsx()` 负责生成描述结构的对象树；真正把树变成 DOM 并挂到容器上的是渲染层（如 `createRoot(...).render()`）。

> 💡 新转换的核心优势：**无需在每个文件中 `import React`**，构建工具会自动注入 `jsx-runtime`。

**旧转换 Classic Runtime（React 16 及更早，了解即可）：**

旧转换将 JSX 编译为 `React.createElement()` 调用，因此每个使用 JSX 的文件都**必须** `import React`，否则运行时报错。维护老项目或阅读旧代码时会遇到这种写法：

```
JSX 源码 → 构建工具转换 → React.createElement(type, props, ...children) → 返回 { type, props } 对象树
```

| 对比项             | 新转换 Automatic Runtime（React 17+）         | 旧转换 Classic Runtime（React 16 及更早）    |
| ------------------ | --------------------------------------------- | -------------------------------------------- |
| **编译目标**       | `jsx()` / `jsxs()`（来自 `react/jsx-runtime`） | `React.createElement()`                     |
| **是否需导入 React** | 不需要，构建工具自动注入                    | 必须手动 `import React from 'react'`         |
| **现代工具默认**   | ✅ Vite、Next.js 等默认启用                    | 需手动配置才会使用                           |
...
> 💡 短语法 `<>...</>` 不支持任何属性。列表渲染中需要给 Fragment 加 `key` 时，必须用完整写法 `<React.Fragment key={item.id}>...</React.Fragment>`。
...
| **key 不是 prop**    | `key` 是 React 内部使用的保留属性，**不会**通过 `props.key` 传入子组件；子组件若需要该值，须另设一个 prop 传入（如 `<Item key={id} id={id} />`） |
...
React 的 `jsx()` 对应 Vue 的 **`h` 函数**（hyperscript），两者都用于创建虚拟 DOM 节点（React 的 JSX 经构建工具转成 `jsx()` 调用）。Vue 的 `h()` 写法：

```js
// Vue 3：h 函数（hyperscript）
import { h } from 'vue'
...
| 职责               | React                                | Vue 3                            |
| ------------------ | ------------------------------------ | -------------------------------- |
| **创建虚拟 DOM**   | `jsx(type, props)`（构建工具自动注入） | `h(type, props, children)`      |
| **挂载到真实 DOM** | `createRoot(el).render(<App />)`     | `createApp(App).mount('#app')`   |

两者遵循同一套思路：**用函数描述 UI 结构（虚拟 DOM）→ Diff 算法比较新旧虚拟 DOM → 最小化更新真实 DOM**。
...
**React 18 自动批处理（Automatic Batching）：** React 17 及更早版本仅在**事件处理函数**中批处理状态更新，`setTimeout`、`Promise.then`、原生事件回调中的多次 `set` 会触发多次渲染。React 18 起，**所有场景**（事件处理、`setTimeout`、`Promise`、原生事件等）中的状态更新都会自动批处理，统一在一次渲染中完成。

| 场景     | 写法 |
| -------- | ---- |
| **基于上一次状态更新** | 使用 **函数式更新**：`setCount(prev => prev + 1)`，避免闭包拿到旧值 |
| **需要立即触发渲染** | 用 `flushSync(() => { setState(...) })` 强制同步更新（极少需要） |
...
## 八、clsx / classnames 优化类名（可选）

多个类名需按条件组合时，用 **clsx** 或 **classnames** 可避免冗长的 `&&` 或模板字符串拼接。

| 方式           | 适用场景       |
| -------------- | -------------- |
| 逻辑与 `&&`    | 单个类名有无   |
| **clsx / classnames 对象语法** | 多个类名按条件组合，更清晰 |

| 库             | 说明 |
| -------------- | ---- |
| **`clsx`**     | 更轻量（< 1KB）、性能更优，当前社区主流选择（shadcn/ui、Tailwind 文档等均推荐） |
| **`classnames`** | 老牌库，API 与 `clsx` 基本一致，仍可用 |
...
// 对象语法：键为类名，值为条件
<div className={clsx('tab', { active: isActive, disabled: isDisabled })} />
```
...
| 对比项   | React                                          | Vue 3                                |
| -------- | ---------------------------------------------- | ------------------------------------ |
| **方案** | 需借助 `clsx` 等第三方库                       | 内置 `:class` 对象/数组语法          |
| **语法** | `clsx('tab', { active: isActive })`            | `:class="{ tab: true, active: isActive }"` |
