...
`createElement` 负责生成描述结构的对象树；真正把树变成 DOM 并挂到容器上的是渲染层（如 `ReactDOM.createRoot().render()`）。
...
| **导航 Tab**     | 状态存当前选中下标（如 `activeIndex`），高亮用 `activeIndex === index`，可用 `lodash` 的 `orderBy` 做排序 |
