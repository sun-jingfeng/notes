...
function Root() {
  ...
  // 用 useMemo 稳定 value 引用，避免 Root 因其他 state 变化重渲染时所有消费者跟着重渲染
...
TS 中 `createContext<T | null>(null)` 使消费处的类型为 `T | null`，每次使用都要判空。实际项目中，将 `createContext`、Provider 组件、自定义消费 Hook 封装在同一个模块中，对外只暴露 Provider 和 Hook；自定义 Hook 内部统一处理 `null` 检查，消费方直接获得完整类型，且无需感知底层 `Context` 对象：
...
export function useTheme() {
  ...
  return ctx // 返回类型为 ThemeContextType，消费方无需判空
}
```

```tsx
// App.tsx — 使用方只需导入 Provider 和 Hook，无需接触 Context 对象
...
| 规则                         | 说明                                                              |
| ---------------------------- | ----------------------------------------------------------------- |
| **只能在顶层调用**           | 不可放在 `if`、`for`、嵌套函数中                                  |
| **只能在 React 函数中调用**  | 只能在**函数组件**或**自定义 Hook** 中调用，不能在普通 JS 函数中调用 |

**原因**：React 在组件的 Fiber 节点上以**链表**存储所有 Hook 状态，依赖每次渲染时 Hooks 的**调用顺序一致**来正确关联状态与 Effect；普通函数没有对应的 Fiber 节点，React 无法追踪其状态。
...
**useId**（React 18+）生成**跨服务端与客户端稳定一致**的唯一 ID，主要用于关联 HTML 可访问性属性（如 `<label htmlFor>` 与 `<input id>`）。
...
| 注意项 | 说明 |
| ------ | ---- |
| **用途** | 生成唯一 ID 用于可访问性属性关联，不用于列表 `key` 或 CSS 选择器 |
| **SSR 安全** | 服务端与客户端生成相同 ID，避免 hydration 不匹配 |
| **多个 ID** | 同一组件需多个 ID 时，在 `useId()` 返回值上加后缀：`id + '-name'`、`id + '-email'` |

> 💡 不要用 `useId` 生成列表的 `key`——`key` 应来自数据本身（如数据库 ID）。`useId` 解决的是"同一组件多次渲染时，每个实例的 HTML `id` 属性需唯一"的问题。

### 3.4 Vue 3 对照

Vue 3 Composition API 的 `ref()`、`computed()`、`watch()` 等虽与 React Hooks 形似，但**没有调用位置和顺序限制**。

| 对比项 | React Hooks | Vue Composition API |
| ------ | ----------- | ------------------- |
| **条件 / 循环中使用** | ❌ 不允许 | ✅ 允许 |
| **普通 JS 函数中使用** | ❌ 不允许，只能在组件或自定义 Hook 中 | ✅ 允许，任意 JS 文件均可 |
| **响应式机制** | Fiber 链表 + 调用索引，依赖顺序一致 | Proxy 依赖追踪，与调用位置和顺序无关 |

```js
// Vue —— 在普通 JS 文件中创建响应式状态，完全合法
...
> 💡 React 状态绑定在组件 Fiber 节点上，脱离组件就无法工作；Vue 的 `ref()` 本质是独立的 Proxy 对象，不依赖任何组件上下文，因此可以在任何位置创建和使用（Pinia 的 store 底层就是利用这一点）。
...
React 通过**组件在树中的位置 + 类型**来决定是否复用组件实例。给组件设置不同的 `key`，React 会将其视为**全新组件**，销毁旧实例并创建新实例，所有内部状态（`useState`、`useRef` 等）随之重置。
...
function App() {
  ...
  return (
    <>
      ...
      {/* key 变化 → 销毁旧 EditForm 并创建新实例 → 内部状态全部重置 */}
...
function EditForm({ userId }: { userId: number }) {
  // userId 切换时 key 变化，name 自动重置为空字符串，无需 useEffect 手动清空
...
| 场景 | 说明 |
| ---- | ---- |
| **表单切换** | 编辑不同数据项时，用数据 ID 作 key，切换即重置表单 |
| **动画重触发** | 改变 key 强制组件重新挂载，触发入场动画 |
| **替代 useEffect 重置** | 比在 useEffect 中监听 props 变化再手动 `setState` 更简洁可靠 |

> 💡 `key` 重置是 React 推荐的模式，适用于"props 变化时需要完全重置内部状态"的场景。比用 `useEffect` 监听 props 并手动重置每个 state 更安全——不会遗漏某个状态。
...
|                                   | **`useState` 的 setter、`useReducer` 的 dispatch**（React 保证引用稳定） |
...
依赖项使用 **`Object.is` 浅比较**，不进行深度对比：

| 类型 | 比较方式 | 触发条件 |
| ---- | -------- | -------- |
| **原始类型**（string、number、boolean） | 值比较 | 值变化即触发 |
| **引用类型**（object、array） | 引用比较 | **引用地址**变化才触发，内部属性变化不触发 |
...
user.name = 'Jerry'
setUser(user)              // ❌ 不触发 —— 同一引用
setUser({ ...user })       // ✅ 触发 —— 新对象，新引用
```

React 没有类似 Vue `watch` 的 `{ deep: true }` 选项。需要监听对象内部变化时，把**具体属性**拆到依赖数组：
...
也可以用 `JSON.stringify` 将整个对象序列化后作为依赖，但每次渲染都要序列化，性能较差，且无法处理循环引用、`undefined`、函数等值，不推荐在生产中使用：

```tsx
useEffect(() => {
  console.log('user deep changed')
}, [JSON.stringify(user)]) // ⚠️ hack，性能差
```

> 💡 Vue 通过 `Proxy` 拦截属性访问，天然支持深度追踪；React 没有响应式代理，只能靠浅比较，因此必须依赖**不可变更新**（创建新引用）来触发副作用。遵循不可变数据模式，就不需要"深度监听"。
...
组件快速切换（如 `id` 连续变化）时，先发出的请求可能后返回，导致旧数据覆盖新数据。用 `AbortController` 在清理函数中取消过期请求：
...
| `useEffect(() => {}, [a, b])` | `watch([a, b], () => {}, { immediate: true })` | 挂载时执行 + 依赖变化时执行 |
...
**useReducer** 适合**多状态联动**或**更新逻辑多分支**的场景，可视为"组件内的迷你 Redux"——把散在各处的 `setState` 收拢到一个 reducer 函数，更易维护和测试。

| 参数            | 说明                                                |
| --------------- | --------------------------------------------------- |
| **reducer**     | 函数 `(state, action) => newState`                  |
| **initialArg**  | 初始 state                                          |
| **init**        | 可选；惰性初始化函数，`init(initialArg)` 得到初始 state |
...
## 七、useImmer — 可变写法管理复杂状态

### 7.1 痛点与原理

React 状态更新必须遵守**不可变原则**，嵌套对象越深，手动展开越繁琐：
...
**[use-immer](https://github.com/immerjs/use-immer)** 是基于 **Immer** 的第三方 Hook 库（Redux Toolkit、Zustand 的 immer 中间件底层也是 Immer），提供 `useImmer` 和 `useImmerReducer`，可以**直接"修改"** draft 对象，Immer 底层通过 Proxy 自动生成不可变新对象：

```bash
npm i immer use-immer
```

### 7.2 useImmer — 替代 useState
...
function ProfileForm() {
  const [formData, updateFormData] = useImmer<ProfileData>({
    name: '',
    age: 0,
    address: { city: '', street: '' },
    tags: []
  ...
  const handleCityChange = (city: string) => {
    updateFormData(draft => {
      draft.address.city = city       // 直接修改嵌套属性
    })
  }

  const addTag = (tag: string) => {
    updateFormData(draft => {
      draft.tags.push(tag)            // 直接 push，无需展开数组
    })
  }
}
```

### 7.3 useImmerReducer — 替代 useReducer

reducer 内可直接修改 state：
...
function reducer(draft: ProfileData, action: Action) {
  switch (action.type) {
    case 'SET_NAME':
      draft.name = action.name                // 直接赋值
      break
    case 'SET_CITY':
      draft.address.city = action.city        // 直接修改嵌套属性，无需展开
      break
    case 'ADD_TAG':
      draft.tags.push(action.tag)             // 直接 push
      break
    case 'RESET':
      return initialProfileData               // 整体替换时用 return
  }
}

const [formData, dispatch] = useImmerReducer(reducer, initialProfileData)
```

### 7.4 原生 vs useImmer 对比

| 对比项       | useState / useReducer                | useImmer / useImmerReducer       |
| ------------ | ------------------------------------ | -------------------------------- |
| **更新方式** | 必须返回全新对象，手动展开每一层     | 直接修改 draft，Immer 自动生成新对象 |
| **嵌套更新** | 层级越深越繁琐                       | 与 Vue `reactive` 体验接近       |
| **额外依赖** | 无                                   | 需安装 `immer` + `use-immer`     |
| **适用场景** | 扁平状态、简单更新                   | 嵌套对象、表单、数组操作频繁     |

> 💡 扁平、简单的状态用原生 `useState` 足矣；嵌套层级深或数组操作频繁时，`useImmer` 能显著减少样板代码。项目已用 Redux Toolkit 或 Zustand immer 中间件时，它们内部已包含 Immer，组件局部状态也可统一用 `use-immer` 保持写法一致。

### 7.5 Vue 3 对照

Vue 3 的 `reactive` 本身就是基于 Proxy 的可变写法，直接修改属性即可触发响应式更新，不需要 Immer 这类工具：
...
// 直接修改，Vue 自动追踪变化并触发渲染
formData.address.city = '北京'
formData.tags.push('新标签')
</script>
```

| 对比项 | React useState | React useImmer | Vue reactive |
| ------ | -------------- | -------------- | ------------ |
| **更新方式** | 手动展开每一层，返回新对象 | 修改 draft，Immer 生成新对象 | 直接修改属性 |
| **底层机制** | 不可变数据 + 浅比较 | Proxy 生成不可变副本 | Proxy 拦截 + 依赖追踪 |
| **额外依赖** | 无 | `immer` + `use-immer` | 无（框架内置） |
| **嵌套更新体验** | 繁琐 | 接近 Vue | 天然支持 |

> 💡 `useImmer` 本质上是让 React 拥有类似 Vue `reactive` 的可变写法体验，但底层仍遵守 React 的不可变原则（Immer 通过 Proxy 记录变更，最终产出全新对象）。Vue 的 `reactive` 则是真正的可变响应式——修改即生效，无需生成新对象。
...
**HOC（Higher-Order Component，高阶组件）** 是一个函数，接收**一个组件**作为参数，返回**一个新的增强组件**，是函数式编程中高阶函数思想在组件层面的应用——对组件能力进行包装扩展，而不修改原组件。命名惯例：`withXxx`（如 `withAuth`、`withLoading`）。

```tsx
// HOC 通用结构
function withXxx<P extends object>(WrappedComponent: React.ComponentType<P>) {
  return function EnhancedComponent(props: P) {
    // 增强逻辑（权限检查、注入 props、添加副作用等）
    return <WrappedComponent {...props} />  // ✅ 必须透传所有 props
  }
}
```
...
// <DataListWithLoading loading={isFetching} items={items} />
```

### 9.3 HOC vs 包裹组件 vs 自定义 Hook

三种模式都能实现"复用逻辑"，形式和用途不同：

| 模式 | 形式 | 使用方式 | 适用场景 |
| ---- | ---- | -------- | -------- |
| **HOC** | `withXxx(Component)` 函数调用 | 在组件**定义阶段**包装，产出新组件 | 需要修改组件的渲染行为（如权限重定向、memo 缓存） |
| **包裹组件** | `<Guard><Layout /></Guard>` JSX 嵌套 | 在 JSX 中声明式包裹 | 条件渲染、提供 Context、布局包裹 |
| **自定义 Hook** | `useXxx()` 函数 | 组件函数体内调用，返回状态/方法 | 复用状态与副作用逻辑，不影响组件结构 |

> 💡 React Router 权限守卫（`<PermissionRoute>`）是**包裹组件**，不是 HOC——使用方式是 JSX 嵌套而非函数调用，两者都常被口语化地称为"高阶组件"，但实现方式不同。HOC 是 React 16.8 之前的主流复用模式；Hooks 出现后，**自定义 Hook** 已基本取代 HOC 承担状态逻辑复用的职责。
...
**设置 displayName：** 被 HOC 包裹后，React DevTools 中组件名默认变为匿名，设置 `displayName` 便于调试：
...
**ref 不透传：** HOC 默认不转发 `ref`，父组件拿到的是包装层的引用。需要透传时，内部用 `forwardRef`（React 18 及以前）或直接从 props 接收 `ref`（React 19）。

***

**多 HOC 嵌套问题：** 多个 HOC 叠加时组件树层级深、props 传递链变长、调试困难（"Wrapper Hell"）。现代项目中优先用自定义 Hook 复用逻辑，HOC 主要留用于需要**干预渲染过程**的场景（如 `memo`、权限重定向）。

### 9.5 React 内置 HOC

| HOC | 说明 |
| --- | ---- |
| **`React.memo`** | 浅比较 props，未变时跳过重渲染 |
| **`forwardRef`**（React 18 及以前） | 使函数组件能接收并转发 `ref` |
| **`connect`**（Redux legacy） | 将 store state / dispatch 注入组件 props |

### 9.6 Vue 3 对照

Vue 3 没有 HOC 模式。逻辑复用通过**组合式函数（Composable）** 实现，渲染增强通过**插槽**或**包裹组件**实现：

| 需求 | React HOC | Vue 3 |
| ---- | --------- | ----- |
| **逻辑复用** | HOC 注入 props | Composable（`useXxx`） |
| **条件渲染 / 守卫** | HOC 内判断后 return | 包裹组件 + `v-if`，或路由守卫 |
| **性能缓存** | `React.memo`（HOC） | Vue 组件默认按需更新，无需手动 memo |

***

## 十、性能优化：React.memo、useCallback 与 useMemo

### 10.1 React.memo

**React.memo** 是一个 HOC，接收函数组件并返回具有**浅比较缓存**能力的新组件：当 props 没有变化时，跳过重新渲染，直接复用上次的渲染结果。

默认情况下，父组件重新渲染时**所有子组件都会跟着重渲染**——即使子组件没有接收任何 props、也没有消费 Context，只要父组件重新执行，子树中的所有组件都会被重新调用。`React.memo` 可以避免这类不必要的渲染。
...
**React Compiler**（原名 React Forget）是 React 官方的编译期优化工具，在构建阶段自动分析组件代码，**自动插入 memoization**——等价于编译器帮你加 `memo`、`useCallback`、`useMemo`。**v1.0 已于 2025 年 10 月正式发布稳定版**。
...
| **兼容性**     | —                                 | 支持 React 17、18、19（对 19 优化最佳） |

实现为 Babel 插件，需要在构建工具中配置启用，并非引入 React 19 就自动生效。Vite、Next.js、Expo、Webpack 等主流构建工具均已支持。Expo SDK 54+ 默认启用；Vite 和 Next.js 的脚手架提供了可选的 Compiler 模板，创建项目时可主动选择。旧项目需手动添加插件配置，可渐进式迁入。

启用后，大部分场景**不再需要**手动编写 `React.memo`、`useCallback`、`useMemo`，编译器会自动完成等价优化。但理解它们的原理仍然重要——有助于理解编译器的行为、排查性能问题、以及维护未启用 Compiler 的项目。

> 💡 安装与配置：https://react.dev/learn/react-compiler/installation

***

## 十一、并发特性：useTransition 与 useDeferredValue
...
> 💡 React 18 中 `startTransition` 回调必须是同步的；**React 19 起支持 async 回调**（异步 Transition），可在回调中 `await` 异步操作，React 自动在整个异步过程中保持 `isPending` 为 `true`。回调内的 `setState` 被标记为低优先级，React 可中断其渲染以响应更紧急的更新。
...
### 11.3 useTransition vs useDeferredValue
...
## 十二、ref 转发与 useImperativeHandle
...
| **React 18 及以前** | 需要用 `forwardRef`（HOC）包裹才能接收 `ref`，React 19 起已不再需要 |

***

**⚠️ 旧版写法（React 18 及以前）：`forwardRef`**

React 19 之前，函数组件无法直接接收 `ref`，必须用 **`forwardRef`**（HOC）包裹，`ref` 作为第二个参数（而非 props 属性）传入。已有项目中大量使用此写法，维护老项目时会频繁遇到：
