...
**Redux** 是 React 生态中最常用的集中状态管理库，可独立于框架使用（Vue 生态中类似工具有 Pinia、Vuex）。
...
| **reducer** | 函数 | 根据当前 state 和 action 计算出新 state，必须为**纯函数**     |
...
> **注意**：reducer 必须是纯函数，不可在 reducer 内发请求或写异步逻辑；异步放在“异步 action”或 **createAsyncThunk** 中。

### 4.2 方式一：手写 thunk（理解流程）
...
// 异步 action：内部请求，再 dispatch 同步 action
...
组件中：`dispatch(fetchList())`，与同步一致。

### 4.3 方式二：createAsyncThunk（推荐）
...
**中间件** 在 **dispatch(action)** 与 **reducer** 之间执行，可用来打日志、处理异步等。RTK 的 **configureStore** 默认已集成 **redux-thunk**，因此可直接在 slice 外写返回函数的异步 action；若需打印 action 日志，可加 **redux-logger** 等中间件。中间件链的顺序有时有影响（如 logger 通常放在最后，便于看到最终 state）。
...
| **reducer 纯函数** | 相同 (state, action) 得到相同结果，不写副作用、不直接改 state；RTK 的 Immer 在 reducer 内允许“可变”写法，底层仍生成不可变新对象 |
...
以“分类 Tab + 商品列表 + 购物车”类应用为例，RTK 与 react-redux 的典型用法归纳如下。

| 功能             | 实现要点 |
| ---------------- | -------- |
| **分类与商品列表** | RTK store 中维护列表状态；异步 action（或 createAsyncThunk）请求接口；组件 `useSelector` 取数并渲染 |
| **Tab 激活**     | store 中存 `activeIndex`；点击分类时 dispatch 更新 `activeIndex`；类名用 `activeIndex === index` 控制高亮 |
| **商品列表切换** | 按 `activeIndex === index` 条件渲染当前分类下的商品 |
...
| 技术点     | 说明 |
| ---------- | ---- |
| **RTK**    | 管理列表数据、activeIndex、cartList 等 |
| **异步 action / createAsyncThunk** | 请求分类与商品数据 |
