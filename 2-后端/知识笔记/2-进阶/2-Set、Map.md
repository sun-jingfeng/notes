...
1. 计算 key 的 `hashCode()`，再通过扰动函数 `(h = key.hashCode()) ^ (h >>> 16)` 降低碰撞概率
2. 取模 `index = hash & (n - 1)` 得到数组下标（n 为数组长度，2 的幂次，位运算等价取模）
3. 若该位置为空，直接存入；否则遍历链表/红黑树：
   - key 相同（`equals` 为 true）→ 覆盖旧值
   - key 不同 → 追加到链表末尾（JDK 1.8 尾插法）
...
| 红黑树节点数 ≤ 6 | 红黑树 → 退化回链表 |
| 链表长度 ≥ 8 但数组长度 < 64 | 触发扩容，不转树 |

**扩容机制**：

- 负载因子默认 `0.75`，扩容阈值 = 容量 × 负载因子（初始为 16 × 0.75 = 12）
- 元素数量超过阈值时，数组 **2 倍扩容**并对所有元素 rehash（重新计算下标）
- 扩容开销大，预估元素数量时应指定初始容量

**线程不安全原因**：

- JDK 1.7：扩容时头插法会形成**环形链表**，导致 `get` 时死循环，CPU 100%
- JDK 1.8：改为尾插法，修复了环形链表问题，但多线程并发 `put` 仍可能**数据覆盖**（两个线程同时写同一桶的末尾）
...
static final int hash(Object key) {
    ...
    // 高 16 位与低 16 位异或，让高位也参与下标计算，降低碰撞
}
```
...
**定义**：线程安全的 `HashMap` 替代品，通过 CAS + `synchronized` 桶级别加锁实现高并发下的线程安全。

**与 `Hashtable` 的本质区别**：

| 对比项 | Hashtable | ConcurrentHashMap |
| --- | --- | --- |
| 锁粒度 | 方法级（整个对象） | 桶级别（只锁当前桶） |
| 并发度 | 串行（同一时刻只有一个线程） | 高并发（不同桶可并行操作） |
| null 支持 | 不允许 | 不允许 |
| 推荐度 | 已过时 | 推荐 |

**JDK 1.8 加锁机制**：

1. **桶为空时**：用 CAS（Compare And Swap）无锁写入，避免加锁开销
2. **桶不为空时**：用 `synchronized` 锁住该桶的头节点，只影响当前桶，其他桶可并行操作

```
并发 put 示意：

线程A → put("a",1) → 落到 bucket[2] → synchronized(bucket[2])
线程B → put("b",2) → 落到 bucket[5] → synchronized(bucket[5])
                                         ↑ 两个线程同时执行，互不干扰
```
...
// 原子操作：不存在时才放入
map.putIfAbsent("张三", 25);  // 不会覆盖，因为 "张三" 已存在
...
**注意事项**：
- 键和值都**不允许为 null**（HashMap 允许），避免并发场景下二义性（null 是"键不存在"还是"值本身是null"无法区分）
- 复合操作（先 `get` 再 `put`）仍需使用 `putIfAbsent` / `compute` 等原子方法，不能拆成两步
