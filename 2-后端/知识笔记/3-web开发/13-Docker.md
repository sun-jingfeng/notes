...
## 十、实战：微服务项目 Docker Compose 部署

**适用场景**：项目由多个 Spring Boot 微服务（如网关、若干业务服务）组成，并依赖 Nacos、MySQL、Redis 等中间件，希望用一份 `docker-compose.yml` 在单机上一键启动整套环境，便于本地联调或测试环境部署。

**典型组成**：

| 角色         | 说明                                       | Compose 中服务名示例   |
| ------------ | ------------------------------------------ | ---------------------- |
| **注册中心** | Nacos（服务注册发现、配置中心）            | `nacos`                |
| **网关**     | Spring Cloud Gateway，统一入口             | `gateway`               |
| **业务服务** | 多个独立 Spring Boot 应用                  | `xxx-service`、`yyy-service` |
| **MySQL**    | 各服务共库或分库，按项目需求               | `mysql`                |
| **Redis**    | 缓存、会话等（可选）                       | `redis`                |

同一 Compose 网络内，各容器通过**服务名**访问，例如 Nacos 地址为 `nacos:8848`、数据库为 `mysql:3306`、Redis 为 `redis:6379`；业务服务配置中填这些主机名即可。

---

### 10.1 目录与镜像约定

在 `/srv` 下按项目名建目录（如 `/srv/my-microservice`），其下按服务分子目录，每个需构建的微服务一个目录，内含 jar 与 Dockerfile；`docker-compose.yml` 放在项目根目录。
...
├── docker-compose.yml
├── mysql/
│   ├── data/                    # 持久化数据，Compose 可挂载命名卷或 ./mysql/data
│   └── init/                    # 可选，初始化 SQL 放此处
├── nacos/                        # 可选：挂载 Nacos 配置或日志
├── gateway/                     # 网关模块
│   ├── Dockerfile
│   └── target/*.jar 或 app.jar
├── xxx-service/                 # 业务服务 A
│   ├── Dockerfile
│   └── target/*.jar 或 app.jar
└── yyy-service/                 # 业务服务 B
...
**各微服务 Dockerfile**（通用 Spring Boot，JDK 17）：jar 在 `target/` 下时用 `COPY target/*.jar app.jar`，与 Dockerfile 同目录时用 `COPY *.jar app.jar`。
...
以下 YAML 定义 Nacos、MySQL、Redis、Gateway、两个业务服务；Gateway 与业务服务通过环境变量指定 Nacos 地址、数据源、Redis 地址。占位符 `<项目名>`、`<MySQL 根密码>`、`<库名>` 等按实际替换。

**启动顺序**：MySQL、Redis、Nacos 先启动；Nacos 就绪后（可用 `healthcheck` 或 `depends_on` + 重试），再启动 Gateway 与业务服务，避免应用启动时连不上注册中心或数据库。
...
services:
  nacos:
    ...
    environment:
      ...
      - MYSQL_SERVICE_PASSWORD=<MySQL 根密码>
    volumes:
...
- Nacos 使用 MySQL 存储配置时，需在 MySQL 中提前建库并执行 Nacos 官方 `schema.sql`（通常在 `mysql/init/` 下放 `nacos-mysql.sql`），否则 Nacos 启动报错；若仅用内存配置可去掉 Nacos 的 `SPRING_DATASOURCE_*` 与 `MYSQL_SERVICE_*`，并去掉对 `mysql` 的 `depends_on`。
- 业务服务若使用 Nacos 配置中心，`bootstrap.yml` 或 `application.yml` 中 `spring.cloud.nacos.config.server-addr` 写 `nacos:8848`；若通过环境变量覆盖，使用 `SPRING_CLOUD_NACOS_CONFIG_SERVER_ADDR`（Spring Boot 2 中对应 `SPRING_CLOUD_NACOS_CONFIG_SERVER_ADDR`）。
- 开启 Nacos 认证时，各 Java 服务需配置 `nacos.auth.identity.key` 与 `nacos.auth.identity.value`（或环境变量 `NACOS_AUTH_IDENTITY_KEY`、`NACOS_AUTH_IDENTITY_VALUE`），与 Nacos 容器内一致。

---

### 10.3 应用配置要点

各微服务在 Compose 网络中通过服务名互相访问，配置中不要写 `localhost`。

| 配置项                     | 本地开发常见值   | Compose 部署时应填   |
| -------------------------- | ---------------- | -------------------- |
| Nacos 地址                 | `localhost:8848` | `nacos:8848`         |
| MySQL 地址                 | `localhost:3306` | `mysql:3306`         |
| Redis 地址                 | `localhost:6379` | `redis:6379`         |
| 其他业务服务（Feign/调用） | `localhost:808x` | 使用**服务名**（如 `http://xxx-service`） |

**方式一**：在 `application-prod.yml` 或 `bootstrap-prod.yml` 中写死上述主机名，激活 `prod` profile 时生效；Compose 中已设 `SPRING_PROFILES_ACTIVE=prod`。

**方式二**：不写死，全部用环境变量覆盖（如 `SPRING_DATASOURCE_URL`、`SPRING_CLOUD_NACOS_DISCOVERY_SERVER_ADDR`），便于同一镜像在不同环境复用。

---

### 10.4 部署步骤

在项目根目录（即含 `docker-compose.yml` 的目录）执行：

```bash
# 1. 进入项目目录（将 <项目名> 替换为实际路径）
cd /srv/<项目名>

# 2. 构建并启动所有服务（首次或代码/镜像变更时加 --build）
docker compose up -d --build
...
docker compose logs -f xxx-service

# 5. 验证：Nacos 控制台
# 浏览器打开 http://<宿主机IP>:8848/nacos ，确认各服务已注册

# 6. 验证：通过网关访问业务接口
curl http://localhost:8080/<网关路由前缀>/xxx/...
```

**仅重建某一服务**（例如只改了 xxx-service）：

```bash
docker compose up -d --build xxx-service
```
...
**停止并删除容器、网络及数据卷**（清空 MySQL/Redis/Nacos 数据）：
...
| 现象                     | 可能原因与处理                                       |
| ------------------------ | ---------------------------------------------------- |
| 业务服务启动报 Nacos 连接失败 | Nacos 未就绪；为 Nacos 配置 `healthcheck`，业务服务 `depends_on: nacos: condition: service_healthy` |
| 业务服务报数据库连接失败 | MySQL 未就绪或库未建；为 MySQL 配置 `healthcheck`，并保证 `mysql/init/` 下已有建库/建表 SQL |
| Nacos 控制台无法打开     | 防火墙未放行 8848/9848；或 Nacos 未启动成功，查 `docker compose logs nacos` |
| 服务间调用超时           | 是否用服务名调用（如 `http://xxx-service/...`），而非 `localhost`；是否在同一网络 `ms-net` |
| Mac Apple Silicon 镜像无法运行 | Nacos 等镜像可加 `platform: linux/amd64` 再试         |
