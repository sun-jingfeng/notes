...
| 能力           | 说明                                       |
| -------------- | ------------------------------------------ |
| **通用 CRUD**  | Mapper 继承 `BaseMapper`、Service 继承 `ServiceImpl`，无需手写单表 CRUD |
| **条件构造器** | `QueryWrapper` / `LambdaQueryWrapper` 链式拼条件 |
| **分页插件**   | 内置分页，无需 PageHelper                   |
| **代码生成器** | 可按表生成 Entity、Mapper、Service、Controller |
...
**配置（沿用 Spring 数据源，可补充 MyBatis-Plus 项）：**
...
public interface UserMapper extends BaseMapper<User> {
    // 已具备：insert、deleteById、updateById、selectById、selectList、selectPage 等
    // 复杂 SQL 仍可在本接口中声明方法，并在 XML 中实现
}
```
...
| 层级       | 作用                                       |
| ---------- | ------------------------------------------ |
| **自定义接口** | 继承 `IService<Entity>`，定义业务方法（含通用 CRUD 与业务扩展） |
| **实现类**   | 继承 `ServiceImpl<Mapper, Entity>`，并 `implements` 自定义接口 |

这样实现类既拥有 `save`、`getById`、`list`、`page`、`updateById`、`removeById` 等通用方法，又在自定义接口中声明业务方法，在实现类中编写具体逻辑。

**自定义 Service 接口（继承 IService）：**

```java
public interface UserService extends IService<User> {
    // 通用 CRUD 已由 IService 提供
    // 仅声明业务扩展方法
    List<User> listByStatus(Integer status);
}
```

**实现类（继承 ServiceImpl 并实现自定义接口）：**
...
**Controller 注入的是接口类型，调用通用方法与业务方法：**
...
public class UserController {
    ...
    public User getById(@PathVariable Long id) {
        return userService.getById(id);  // 通用方法
    }

    @GetMapping("/status/{status}")
    public List<User> listByStatus(@PathVariable Integer status) {
        return userService.listByStatus(status);  // 业务方法
    }
}
```
...
| 要点         | 说明                                             |
| ------------ | ------------------------------------------------ |
| **接口**     | 继承 `IService<Entity>`，便于扩展和替换实现       |
| **实现类**   | 继承 `ServiceImpl<Mapper, Entity>` 并实现该接口  |
| **通用能力** | 来自 `ServiceImpl`，无需手写单表 CRUD            |
| **业务逻辑** | 在自定义接口中声明，在实现类中用 `lambdaQuery()` 等编写 |
...
```java
@Data
@TableName("sys_user")  // 表名（默认类名驼峰转下划线）
public class User {
...
```java
// 等价于: WHERE status = 1 AND name LIKE '%张%' ORDER BY create_time DESC
